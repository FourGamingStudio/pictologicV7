...
core.build = () => {
	...
	d.cont.pane(t => {
		...
		t.button("$Display", icon, () => {
			ui.select("$SelectDisplay", displays, d => {
				core.display = d;
		...
		t.check("$GrayTransparency", core.useGray, b => {core.useGray = b})
			.growX().center().row();
		...
		quality.row();
		quality.check("$UseHSV", core.hsv, b => {core.hsv = b})
